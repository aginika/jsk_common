...
def checkLocalRemoteROSParamDiff(param_name, host, remote_param_name="", ok_message="", error_message=""):
    if not rospy.has_param(param_name):
        errorMessage("Local doesn't have " + param_name)
        return False

    target_local_param_md5 = subprocess.check_output("bash -c 'source ~/ros/hydro/devel/setup.bash ;rosrun jsk_tools calc_md5.py " + param_name + " '", shell=True)
    target_remote_param_md5 = subprocess.check_output(["ssh", host,  "source ~/ros/hydro/devel/setup.bash; rosrun jsk_tools calc_md5.py " + (remote_param_name if remote_param_name else param_name)])
